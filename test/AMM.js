...
beforeEach(async () => {
  ...
  investor1 = accounts[2] // token1 owner
  investor2 = accounts[3] // token2 owner

  // Deploy Token
  const Token = await ethers.getContractFactory('Token')
  token1 = await Token.deploy('Dapp University', 'DAPP', '1000000') // 1 Million
  token2 = await Token.deploy('USD Token', 'USD', '1000000') // 1 Million

  // Send funds to liquidity provider
  ...
  describe('Deployment', () => {
    ...
    it('returns token1 address', async () => {
      expect(await amm.token1()).to.eq(token1.address)
    })

    it('returns token2 address', async () => {
      expect(await amm.token2()).to.eq(token2.address)
    })
  })
  ...
  describe('Swapping tokens', () => {
    ...
    it('facilitates swaps', async () => {
      ...
      // AMM receives tokens
      expect(await token1.balanceOf(amm.address)).to.eq(amount)
      expect(await token2.balanceOf(amm.address)).to.eq(amount)

      expect(await amm.token1Balance()).to.eq(amount)
      expect(await amm.token2Balance()).to.eq(amount)

      // console.log(await amm.K())

      // Check deployer has 100 shares
      expect(await amm.shares(deployer.address)).to.eq(tokens(100)) // use tokens helper to calculate shares
      ...
      // Deployer shoudl still have 100 shares
      ...
      // Estimate amount of tokens investor1 will receive after swapping token1: include slippage
      estimate = await amm.calculateToken1Swap(tokens(1))
      console.log(`Token2 Amount investor1 will receive after swap (include slippage): ${ethers.utils.formatEther(estimate)}`)
