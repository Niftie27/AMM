...
const Charts = () => {
  const provider = useSelector(state => state.provider.connection) // fetch from component state
  ...
  useEffect(() => {
    if(provider && amm){
      loadAllSwaps(provider, amm, dispatch) // loads entire swap history from amm
    }
  }, [provider, amm, dispatch]); // loads entire swap history from amm 
          ...
          <Table striped bordered hover>
            ...
            <tbody>
              ...
              <tr key={index}>
                ...
                <td>{// new to convert timestamp to miliseconds for Javascript .toLocaleDateString(undefined, year...)
